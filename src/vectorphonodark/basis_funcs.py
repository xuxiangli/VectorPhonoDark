"""Linear and logarithmic Haar wavelet basis functions.

MIRROR: these primitives are reimplemented as in the "Haar Wavelet Functions"
section of ``analytic_cy.pyx``. Any correction to the basis here must be
...
def haar_support(n: int) -> list[float]:
    ...
    x_min = 2.0 ** (-lam) * (mu + 0.0)
    x_mid = 2.0 ** (-lam) * (mu + 0.5)
    x_max = 2.0 ** (-lam) * (mu + 1.0)
...
def haar_support_log(n: int, eps: float) -> list[float]:
    ...
    length = math.log(1.0 / eps)

    x_min = eps * math.exp(2.0 ** (-lam) * (mu + 0.0) * length)
    x_mid = eps * math.exp(2.0 ** (-lam) * (mu + 0.5) * length)
    x_max = eps * math.exp(2.0 ** (-lam) * (mu + 1.0) * length)
    return [x_min, x_mid, x_max]
...
def haar_value(n: int, dim: int = 3) -> list[float]:
    ...
    """
    if n == 0:
        # No second (b) region at n=0; the -b_n slot is a placeholder. Sign it
        # to match the [a_n, -b_n] convention and the analytic_cy.pyx mirror.
        # Unused in practice: every caller reads only value[0] at n=0.
        return [math.sqrt(dim), -math.sqrt(dim)]
...
def haar_value_log(n: int, eps: float, p: int = 2) -> list[float]:
    ...
    eps : float
        The lower bound of the integral.
    p : int
        The weight in orthonormality condition int x^p dx (default is 2).
    ...
    if n == 0:
        # No second (b) region at n=0; the -b_n slot is a placeholder, signed to
        # match the [a_n, -b_n] convention and the analytic_cy.pyx mirror.
        # Unused in practice: every caller reads only value[0] at n=0.
        match p:
            case 0:
                a0 = math.sqrt(1.0 / (1 - eps))
            case -1:
                a0 = math.sqrt(1.0 / math.log(1.0 / eps))
            case _:
                a0 = math.sqrt((p + 1.0) / (1.0 - math.pow(eps, p + 1.0)))
        return [a0, -a0]
    ...
    rho = math.exp(length * 2.0 ** (-(lam + 1)))
    match p:
        case 0:
            a_n = math.sqrt(1.0 / (x1 * (rho - 1.0) * (1.0 + 1.0 / rho)))
            b_n = math.sqrt(1.0 / (x1 * rho * (rho - 1.0) * (1.0 + rho)))
        case -1:
            a_n = b_n = math.sqrt(2.0**lam / length)
        case _:
            b_n = math.sqrt(
                (p + 1.0)
                / (
                    math.pow(x1, p + 1.0)
                    * (math.pow(rho, p + 1.0) - 1.0)
                    * (math.pow(rho, p + 1.0) + 1.0)
                    * math.pow(rho, p + 1.0)
                )
            )
            a_n = b_n * math.pow(rho, p + 1.0)
    return [a_n, -b_n]
...
@numba.njit
def haar_func_log(n: int, x: float, eps: float, p: int = 2) -> float:
    """

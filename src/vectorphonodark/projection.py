...
def generate_mesh_ylm_jacob(u_max, n_r, n_theta, n_phi,
    ...
    theta_list = np.arccos(
        -np.linspace(-1. + dcostheta / 2, 1. - dcostheta/2, n_theta)
    )
    dphi = 2 * np.pi / n_phi
...
def proj_form_factor(n_max, l_max,
    ...
    if q_cut_option:
        q_cut = physics.compute_q_cut(
            phonon_file, phonopy_params['atom_masses'])
        if q_cut < q_max:
            q_max = q_cut
            if verbose:
                print(
                    f"    Adjusted q_max to {q_max:.4f} eV due to Debye Waller factor.")
        else:
            if verbose:
    ...
    if verbose:
        ...
        print(
            f"    Grid rescaled: power_r={power_r}, power_theta={power_theta}, power_phi={power_phi}")
        print(
